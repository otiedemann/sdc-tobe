#!/usr/bin/env bash
# cloud-bootstrap.sh — go from a fresh Ubuntu 22.04 GPU server
# to a running sdc-sphinx container in one command.
#
# Works on any provider that gives you a Linux VM with root SSH and
# an NVIDIA GPU. Sphinx needs `--privileged` capabilities, so this
# does NOT work on container-as-a-service providers (RunPod, Modal,
# Beam, Salad, Replicate) — only on real VMs / bare metal.
#
# Tested on:
#   - DataCrunch (Finland, EU; ~0.40 €/h for RTX 4090)
#   - Lambda Labs (US; A10 / A100)
#   - Hetzner Dedicated (DE; AX-line with consumer GPUs, ~80 €/Mt)
#   - OVHcloud (FR; T2 / L40S instances)
#   - Vast.ai interactive (filter "verified" + "Allowed: docker run")
#   - bare metal with NVIDIA card + Ubuntu 22.04
#
# Usage:
#
...
#
#   ↑ pass the image as $1; -E preserves env vars through sudo.
#
# Env vars (optional — script prompts via /dev/tty if missing):
#   GHCR_USER       Username for ghcr.io login (default: otiedemann)
#   GHCR_PAT        GitHub PAT with read:packages scope (required for private images)
