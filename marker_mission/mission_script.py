...
class Step:
    ...
    # Loose arrival tolerance (m) for an GO_HOME step: the approach is
    ...
    # Optional yaw-settle tolerance (deg) for APPROACH's 3rd arg: how far OFF
    # facing the marker the drone may be and still ADVANCE. None -> the tight
    # default (waits to be head-on, 0°). A wide value (e.g. 45) makes APPROACH
    # advance the instant the target DISTANCE is reached and the drone is within
...
def parse(text: str, defaults: dict) -> List[Step]:
    ...
    for raw_line_no, raw in enumerate(text.splitlines(), start=1):
        ...
        if cmd == "TAKEOFF":
            ...
            dist = (_parse_float(args[1], raw_line_no, "GO_HOME distance")
                    if len(args) >= 2 else 3.5)
            tol = (_parse_float(args[2], raw_line_no, "GO_HOME tolerance")
